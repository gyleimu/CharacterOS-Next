...
**状态:** 骨架阶段文档。本文件**不移动、不复制、不修改**任何旧文件；只定义分类与迁移计划。任何实际迁移必须逐项经 ROADMAP G3 门禁（单项审批 + 来源 SHA 记录）。
...
| event-centric state transition 语义 | 事件驱动状态转移 | **REWRITE** | 新 causal loop（ARCHITECTURE §3） | 约束 A 缺口 #1（顺序错误）与 #4（无闭环） |
| Emotion 模块（非持久） | 当前 emotion 非 persistent state | **REWRITE** | packages/affect（V0 = FAST+EMA minimal persistence） | 约束 A 缺口 #2 + 约束 B（FAST+EMA 产品身份） |
| Life state 持久化（prisma 等） | 未完整跨 tick 持久化 | **REWRITE**（设计级 ADAPT 持久化经验） | packages/subject-core 持久层设计 | 约束 A 缺口 #3 |
| Behavior pipeline（src/character 等 Track A 侧） | 未形成 Action→World→Consequence→Experience 闭环 | **REWRITE** | packages/behavior + runtime（闭环） | 约束 A 缺口 #4 |
| Memory retrieval | 不够 autobiographical/context-sensitive | **REWRITE** | packages/memory（NEXT_ACTIONS #3） | 约束 A 缺口 #5 |
| Relationship 模块 | 尚非一等长期状态 | **REWRITE** | packages/relationship（九层状态之一） | 约束 A 缺口 #6 |
| Personality / trust / attachment / fear | 时间尺度混层 | **REWRITE**（分层设计） | packages/personality + 九层分层规则 | 约束 A 缺口 #7 |
...
> 依据：约束 B（Emotion 全路线审计）+ EXPERIMENT_INDEX/E0–E39 谱系。

| 资产 | 内容 | 分类 | 去向 | 依据/理由 |
|---|---|---|---|---|
| `experiments/emotion-plasticity/`（FAST+EMA、COUNTER、INTENSITY） | minimal core 与基线族 | **ADAPT**（V0 机制实现参考）+ **BASELINE_ONLY**（研究侧身份） | packages/affect（设计参考）+ evals/baselines/fast-ema | 约束 B：FAST+EMA = minimal persistence + BASELINE_ONLY；Plasticity Phase 1 verdict B |
...
| Phase 5（5.0–5.2 因果链基准） | 事件→记忆→状态→行为链 | **ADAPT**（链本体结论）+ **BASELINE_ONLY**（5.2 基准测量待修复） | 新 causal loop 设计 + evals/regression | 约束 A/B：链本体 90–100% 通过；测量设计产物待修 |
...
```text
KEEP         : 旧产品应用层、CERH v0.1 工具
ADAPT        : kernel 机制库、FAST+EMA、Phase 5 链结论、TED 归因结论、8 条教训、appraisal benchmark
REWRITE      : 事件语义、emotion 持久化、life state 持久化、行为闭环、记忆检索、关系、时间尺度分层
BASELINE_ONLY: FAST+EMA、appraisal benchmark、Phase 5.2（测量修复后）
RESEARCH_HYPOTHESIS: TED Q18 objective-design、E5 机制、T1–T8 触发问题
