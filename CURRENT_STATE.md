...
`DEEPSEEK_EXECUTOR_LONG_RUN_V0`（Human decision：改用真实 DeepSeek API 作为产品 executor；只换 executor，不修 context budget，不改 core）已完成**除真实运行以外的全部内容**，真实 20 轮运行被 credential 阻塞：recon 确认产品已内建 cloud family（`CHARACTEROS_EXECUTOR=deepseek|ollama|auto`，`OpenAiCompatibleTransportV0` → `POST {MODEL_API_BASE_URL}/chat/completions`，默认 model `deepseek-flash`、endpoint `https://api.deepseek.com`，credential **只**从环境变量 `MODEL_API_KEY` 读取且只暴露 presence），**没有新增任何 provider**；但本机 `MODEL_API_KEY` 在 Process / User / Machine 三个层级都不存在，因此 `CHARACTEROS_EXECUTOR=deepseek` 会被产品 **fail closed** 拒绝（`core-v1-long-run-blocked-v0` 写入 `tmp/deepseek-long-run-blocked.json`），harness 报 `DEEPSEEK_RUN_BLOCKED_CREDENTIAL_ABSENT` 并且在任何 interaction 之前停止 —— 不伪造、不从文件/参数取 credential。本 slice 不修改 core（`core_changes: []`），也不触碰 context budget / retrieval / prompt（§4：先只换 executor，保留干净对照）。

本轮补上的 monitoring instrumentation（observability，不是 core change；`core_changes: []`）：新增 `product/sandbox/src/provider-request-observer.ts` —— fetch 层观察器，记录**精确** provider HTTP 请求数、HTTP status、每次调用耗时、request/response **字节数**、model、output-token budget 与 provider 自报的 `usage`（prompt/completion/total tokens；provider 未返回时如实记 `NOT_AVAILABLE`，绝不估算）；**永不记录 header（authorization 从不读取）、body、prompt、memory 内容或模型输出**，观察永不改变调用语义且不向调用路径抛错。long-run harness 相应升级：checkpoint 命名为 BEFORE / TURN_5 / TURN_10 / TURN_15 / TURN_20 / FINAL_RESTORE；每轮记录 wall latency、失败分类（EXECUTOR / PRODUCT / CORE，executor 失败绝不归到 core）、失败轮 durable 不变性检查；连续 2 次同类 context failure 立即 STOP（不烧剩余调用）；cloud family 下 artifact 命名为 `deepseek-long-run-checkpoint-<N>.json`；per-stage 调用数取自**既有**产品 diagnostics（`diagnosticsView()`，进程内成功样本）。离线验证（0 real model calls）：observer 单测 4/4（计数、usage 提取、pass-through、无 credential 泄漏）、executor selection 14/14、以及一次针对**本地 stub provider** 的 plumbing dry run（一次性 subject root、`CHARACTEROS_DISABLE_ADAPTATION=1`、stub 计 2 请求 / observer 精确计 2 / usage 2103 tokens / artifact 命名正确）—— 真实调用前的管线全部验证完毕，真实运行只需环境里存在 `MODEL_API_KEY` 后执行：`CHARACTEROS_EXECUTOR=deepseek CHARACTEROS_LONG_RUN=1 npx vitest run product/sandbox/src/long-run-checkpoint.test.ts`（继续同一 `alice-longrun`：rev 71 / R23 / 8 episodes）。
