...
下一个产品 slice 是 `CHARACTEROS_PRODUCT_CONFIGURATION_AND_ONBOARDING_UX_V0`：read-only `/config`、首次运行引导与失败后可操作指引，不引入新 canonical state。该 slice 未启动，也不由本文件授权启动。下一个产品 slice 是 `CHARACTEROS_PRODUCT_CONFIGURATION_AND_ONBOARDING_UX_V0`：read-only `/config`、首次运行引导与失败后可操作指引，不引入新 canonical state。该 slice 未启动，也不由本文件授权启动。
