...
function checkpointFileName(subjectId: string): string {
  return `subject-${subjectId}.environment-checkpoint.json`;
}
...
export class FileEnvironmentCheckpointStoreV0 implements EnvironmentCheckpointStoreV0 {
  ...
  constructor(rootDir: string, subjectId: string) {
    this.path = join(rootDir, checkpointFileName(subjectId));
  }
  location(): string {
    return this.path;
  }
