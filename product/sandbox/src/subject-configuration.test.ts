    ...
    complete: async (request: ModelTransportRequestV0): Promise<ModelTransportResponseV0> => {
      ...
      return {
        content: JSON.stringify({
          response_semantics: selected === "REALIZE" ? { kind: "PRIMARY_CONVERSATIONAL_ACT", act: "ACKNOWLEDGE" } : { kind: "PRIMARY_CLARIFICATION" },
...
describe("PERSISTENT_SUBJECT_CONFIGURATION_V0 — configuration + resolution", () => {
  ...
  it("provider prompt semantics carry the V2 authority contract and the display name never enters the prompt", async () => {
    ...
    expect(request?.messages[0]?.content).toBe(CONVERSATION_COGNITION_SYSTEM_PROMPT_V8);
