...
export interface RuntimeDependencyContainer {
  ...
  /**
   * PRE_COGNITION_CANONICAL_APPRAISAL_V0 — the frozen conversation factual
   * event authority (Ingress ↔ committed Observation ↔ event_ref). null = not
   * wired; the factual-event appraisal executor fails closed.
   */
  readonly factualEventAuthority: ConversationFactualEventAuthorityV0 | null;
  /**
   * PRE_COGNITION_CANONICAL_APPRAISAL_V0 — factual-event appraisal provider
   * port. Proposes ONLY dimensions/confidence/evidence over the frozen
   * pre-cognition context. null = not wired; the executor fails closed.
   */
  readonly factualEventAppraisalProvider: FactualEventAppraisalProviderV0 | null;
}
