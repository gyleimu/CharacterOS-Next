/**
 * ConversationCognitionProviderV7 — live closed factual authority.
 *
 * V1–V6 remain historical. V7 is the only live canonical-projection provider:
...
export const COGNITION_INVOCATION_BINDING_SCHEMA_VERSION_V3 =
  "cognition-invocation-binding-v3" as const;
export const COGNITION_INVOCATION_BINDING_HASH_PROJECTION_V3 =
  "characteros-next/runtime/cognition-invocation-binding/v3" as const;

export interface CognitionInvocationBindingV3 {
  readonly schema_version: typeof COGNITION_INVOCATION_BINDING_SCHEMA_VERSION_V3;
  readonly subject_id: string;
  readonly source_revision: number;
  readonly proposal_schema_version: "conversation-cognition-proposal-v7";
  readonly projection_hash: HashV1;
}

export async function deriveCognitionInvocationBindingHashV3(
  binding: CognitionInvocationBindingV3
): Promise<HashV1> {
  return hashEnvelope(COGNITION_INVOCATION_BINDING_HASH_PROJECTION_V3, binding);
}

const COGNITION_WIRE_JSON_SCHEMA_V7 = Object.freeze({
  type: "object",
...
const SUBJECTIVE_SELECTION_V1_JSON_SCHEMA_V7 = Object.freeze({
  oneOf: [
    {
      type: "object",
      additionalProperties: false,
...
export const CONVERSATION_COGNITION_PROPOSAL_V7_JSON_SCHEMA: Readonly<Record<string, unknown>> =
  Object.freeze({
    type: "object",
    additionalProperties: false,
    required: [
    ...
    properties: {
      schema_version: { const: "conversation-cognition-proposal-v7" },
      factual_assessment: {
        type: "object",
        additionalProperties: false,
        required: ["claims"],
      ...
      cognition: COGNITION_WIRE_JSON_SCHEMA_V7,
      subjective_selection: SUBJECTIVE_SELECTION_V1_JSON_SCHEMA_V7,
      communication_directive: {
        type: "object",
        additionalProperties: false,
        required: ["kind"],
 ...
 */
export const CONVERSATION_COGNITION_SYSTEM_PROMPT_V7 = CONVERSATION_COGNITION_SYSTEM_PROMPT_V6
...
export type ConversationCognitionRejectionCodeV7 =
  | "INVOCATION_BINDING_INVALID"
  | "MODEL_SCHEMA_INVALID"
  | "FACTUAL_AUTHORIZATION_REJECTED";

export class ConversationCognitionRejectionErrorV7 extends Error {
  readonly code: ConversationCognitionRejectionCodeV7;
  readonly factual_authorization_trace: readonly FactualClaimAuthorizationTraceV0[];

  constructor(
    code: ConversationCognitionRejectionCodeV7,
    detail: string,
    trace: readonly FactualClaimAuthorizationTraceV0[] = Object.freeze([])
  ) {
    super(`CONVERSATION_COGNITION_${code}: ${detail}`);
    this.name = "ConversationCognitionRejectionErrorV7";
    this.code = code;
    this.factual_authorization_trace = trace;
  }
}

export class ConversationCognitionProviderV7 {
  private lastProposal: ConversationCognitionProposalV7 | null = null;
  ...
  get lastConversationProposal(): ConversationCognitionProposalV7 | null {
  ...
  async propose(projection: CognitiveContextProjectionAnyVersion): Promise<ConversationCognitionProposalV7> {
    const binding: CognitionInvocationBindingV3 = Object.freeze({
      schema_version: COGNITION_INVOCATION_BINDING_SCHEMA_VERSION_V3,
      subject_id: projection.subject_id,
      source_revision: projection.state_revision,
      proposal_schema_version: "conversation-cognition-proposal-v7",
      projection_hash: projection.projection_hash
    });
    const bindingHash = await deriveCognitionInvocationBindingHashV3(binding);
    if (this.inflightBindings.has(bindingHash) || this.completedBindings.has(bindingHash)) {
      throw new ConversationCognitionRejectionErrorV7(
        "INVOCATION_BINDING_INVALID",
    ...
    try {
      const response = await this.transport.complete({
        messages: [
          { role: "system", content: CONVERSATION_COGNITION_SYSTEM_PROMPT_V7 },
          { role: "user", content: buildConversationSubjectDataV4(projection) }
        ],
        structured_output: { kind: "JSON_SCHEMA", schema: CONVERSATION_COGNITION_PROPOSAL_V7_JSON_SCHEMA }
      });
  ...
  private parse(
  ...
  ): ConversationCognitionProposalV7 {
    let parsed: unknown;
    try {
    ...
    if (!isRecord(parsed)) {
      throw new ConversationCognitionRejectionErrorV7("MODEL_SCHEMA_INVALID", "provider output: expected object");
    }
    ...
    if (!checked.ok) {
      ...
      throw new ConversationCognitionRejectionErrorV7(
        factualRejected ? "FACTUAL_AUTHORIZATION_REJECTED" : "MODEL_SCHEMA_INVALID",
        checked.detail,
