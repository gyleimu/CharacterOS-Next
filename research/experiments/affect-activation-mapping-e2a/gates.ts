/**
 * ACTIVATION_MAPPING_ABLATION_E2A — engineering gates (E1/E2 conventions).
 */
...
export async function runEngineeringGates(argvOutput?: string): Promise<{ output: string; gates: GatesE2A }> {
  const output = freshDirectory(argvOutput ?? "tmp/affect-activation-mapping-e2a-gates");
  ...
  run("prior-line-regressions", process.execPath, ["node_modules/vitest/vitest.mjs", "run",
    "evals/conformance/affect-state-retention-e1.test.ts", "evals/conformance/affect-production-shaped-e2.test.ts"]);
  ...
  check(equal(integrity.production_diff, "EMPTY"), "production and all non-E2A files frozen vs baseline");

  const gates: GatesE2A = {
