/**
 * STATE_RETENTION_AND_RECOVERY_E1 — engineering gates (v1 experiment conventions).
 * Workspace build/typecheck in dependency order (direct Node tsc entrypoints
 * because this Windows host's pnpm child .bin lookup is unreliable), harness
 * typecheck, conformance tests, full production suite, changed-file lint with
 * zero messages, full lint equal to the current zero-debt baseline, and
 * git diff --check from the frozen baseline commit.
 */
...
export async function runEngineeringGates(argvOutput?: string): Promise<{ output: string; gates: GatesE1 }> {
  const output = freshDirectory(argvOutput ?? "tmp/affect-state-retention-e1-gates");
  ...
  function run(name: string, executable: string, args: string[]): void {
    const result = spawnSync(executable, args, { cwd: ROOT, encoding: "utf8", windowsHide: true, timeout: 600000, maxBuffer: 64 * 1024 * 1024 });
  ...
  // Same tsc scripts as the workspace package.json scripts, in dependency order.
  ...
  // Current repository reality: the two historical Regulation unused-var debts
  // are fixed; full lint must be exactly empty (no new debt permitted).
  ...
  check(equal(integrity.production_diff, "EMPTY"), "production files frozen vs baseline");
  ...
  writeFileSync(join(output, "SUMMARY.md"),
    `# E1 engineering gates\n\nPASS. ${commands.length} commands green; changed lint clean; full lint zero debt; production frozen at ${BASELINE_COMMIT}.\n\nSource: ${gates.source_fingerprint}\nBuilt: ${gates.built_fingerprint}\n`,
    { flag: "wx" });
  console.log(output);
  return { output, gates };
}

// Run directly: `node research/experiments/affect-state-retention-e1/gates.ts <outdir>`.
// Imported by cli.ts, which invokes runEngineeringGates itself.
