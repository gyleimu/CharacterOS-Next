...
/**
 * FAMILIARITY_CONTEXT_MEDIATION_REPLICATION_V1 — world construction.
 *
 * SAME-CORPUS INVARIANT: every cell builds the IDENTICAL repository revision (8
 * canonical episodes: 6 counterpart items including the two shared-context items, one
 * generic item, one distractor). Only the number of ADMITTED familiarity receipts
 * differs, plus — for D_LOW_CONTEXT_EQUALIZED only — the canonical working refs, set to
 * exactly the counterpart refs B_HIGH's priority retrieval selects.
 *
 * Familiarity is NEVER assigned and NEVER seeded: it accrues only through real occurred
 * interaction experience -> real ingestion -> real governed writer authority -> Atomic
 * Commit V2.
 *
 * SEED TRUST BOUNDARY (required gate): every world records the exact
 * observeSeededGovernedRelationshipStateV0 observation over the INITIALIZATION seeds.
 *   seed_contains_governed_relationship_state === false
 *   seeded_bundles_with_writer_authority      === 0
 *   unreadable_seed_shapes                    === 0
 * Any failure => EXPERIMENT_INVALID.
 */
...
/** The SAME candidate corpus for every condition: 8 episodes. */
export function corpusEpisodes(): readonly EpisodicMemoryRecordV0[] {
  const interactions = COUNTERPART_EPISODES.map((entry) =>
    episodeRecord(entry.ref, entry.scene, [ALICE])
  );
...
/** Only the counterpart interaction episodes are candidates for familiarity accrual. */
...
/** One real governed writer product observed in the committed canonical history. */
export interface GovernedAuthorityRecordRef {
  readonly credit_index: number;
  readonly state_revision: number;
  readonly commit_ref: string;
  readonly authority_payload_hash: string;
  readonly evidence_receipt_refs: readonly string[];
}

export interface SeedContaminationCheck {
  readonly field_names: {
    readonly contains: "seed_contains_governed_relationship_state";
    readonly bundles_with_authority: "seeded_bundles_with_writer_authority";
    readonly unreadable: "unreadable_seed_shapes";
  };
  /** Observation over the INITIALIZATION seeds (genesis snapshot + seedBundles). */
  readonly initialization_seeds: SeededGovernedRelationshipStateObservationV0;
...
export interface HistoryBundle {
  ...
  readonly seed_contamination: SeedContaminationCheck;
  readonly governed_authority_records: readonly GovernedAuthorityRecordRef[];
...
export async function buildRepository(): Promise<{ repo: InMemoryMemoryRepository; binding: RepositoryRevisionBindingV1; records: readonly EpisodeRecordEntry[]; digest: string }> {
  ...
  const prepared = await repo.prepareRevisionForIntent({
    intent_id: "intent-familiarity-context-mediation-corpus" as never,
...
function seedState(repositoryRevision: string, workingRefs: readonly string[]): SubjectStateV0 {
...
/**
 * Baseline working refs: ONLY the generic item. D_LOW_CONTEXT_EQUALIZED is built through
 * an explicit override once B_HIGH's retrieval selection is known (see cli.ts prepare).
 */
export function workingRefsFor(condition: ConditionId): readonly string[] {
  if (condition === "D_LOW_CONTEXT_EQUALIZED") {
    throw new Error("D_LOW_CONTEXT_EQUALIZED working refs must be supplied explicitly from B_HIGH's retrieval selection");
  }
  return [GENERIC_REF];
}

/** Build a condition's world: real governed head commit + real familiarity ingestion
 * over the IDENTICAL corpus; only the admitted receipt count (and the named `E`
 * working-ref intervention) differs. */
export async function buildHistory(condition: ConditionId): Promise<{ runtime: Runtime; bundle: HistoryBundle }> {
  return await buildHistoryWithCredits(CONDITIONS[condition].credits, condition, workingRefsFor(condition));
}

export async function buildHistoryWithCredits(
  credits: number,
  label: string,
  workingRefs: readonly string[] = [GENERIC_REF]
): Promise<{ runtime: Runtime; bundle: HistoryBundle }> {
  const { repo, binding, records, digest } = await buildRepository();
  const seed = seedState(binding.repository_revision, workingRefs);
  ...
  // ---- REQUIRED seed trust-boundary gate over the INITIALIZATION seeds -------------
  const initialSeedBundles: readonly unknown[] = [];
  const seedObservation = observeSeededGovernedRelationshipStateV0({
    snapshots: [genesis as unknown],
    bundles: initialSeedBundles
  });
  const seedContamination: SeedContaminationCheck = {
    field_names: {
      contains: "seed_contains_governed_relationship_state",
      bundles_with_authority: "seeded_bundles_with_writer_authority",
      unreadable: "unreadable_seed_shapes"
    },
    initialization_seeds: seedObservation,
    seed_contains_governed_relationship_state: seedObservation.seed_contains_governed_relationship_state,
    seeded_bundles_with_writer_authority: seedObservation.seeded_bundles_with_writer_authority,
    unreadable_seed_shapes: seedObservation.unreadable_seed_shapes,
    clean:
      seedObservation.seed_contains_governed_relationship_state === false
      && seedObservation.seeded_bundles_with_writer_authority === 0
      && seedObservation.unreadable_seed_shapes === 0
  };
  check(seedContamination.clean, `seed contamination gate (${label}): ${JSON.stringify(seedObservation)}`);

  const assembly = createInMemorySubjectCoreFacadeForExplicitV4V0({
    seedSnapshots: new Map([[SUBJECT as never, genesis as never]]),
    seedBundles: initialSeedBundles as never,
  ...
  const headProposal = {
    ...
    transition_id: `t-fam-ctxmed-head-${label}`,
    subject_id: SUBJECT,
  ...
  const committed = await assembly.facade.commitReserved({
    ...
    preparedBinding: {
      ...
      prepared_result_ref: `workflow:fam-ctxmed-head-${label}` as never
    },
  ...
  // ---- real governed writer products actually present in the committed history ------
  const governedAuthorityRecords: GovernedAuthorityRecordRef[] = [];
  for (const entry of assembly.storeRead.getCommittedBundles() as unknown as readonly AtomicCommitBundleAnyVersion[]) {
    const authority = (entry as { writer_authority?: unknown }).writer_authority;
    if (authority === null || authority === undefined) continue;
    const typed = authority as {
      writer_family?: string;
      authority_payload_hash?: string;
      authority_payload?: { evidence_receipt_refs?: readonly string[]; dimension_id?: string };
    };
    if (typed.writer_family !== "RELATIONSHIP_GOVERNED_FEATURE") continue;
    if (typed.authority_payload?.dimension_id !== DIMENSION_ID) continue;
    governedAuthorityRecords.push({
      credit_index: governedAuthorityRecords.length + 1,
      state_revision: entry.next_revision as number,
      commit_ref: entry.commit_ref as string,
      authority_payload_hash: typed.authority_payload_hash as string,
      evidence_receipt_refs: typed.authority_payload?.evidence_receipt_refs ?? []
    });
  }
  check(
    governedAuthorityRecords.length === credits,
    `governed authority records for ${label}: ${governedAuthorityRecords.length} (expected ${credits})`
  );
  ...
  const bundle: HistoryBundle = {
    ...
    seed_contamination: seedContamination,
    governed_authority_records: governedAuthorityRecords,
    working_refs: [...workingRefs]
  };
...
export function familiarityOf(snapshot: SubjectStateV4): number | null {
  ...
  const dimension = counterpart?.dimensions.find((entry) => entry.dimension_id === DIMENSION_ID);
 ...
 * NOTE: restore legitimately re-seeds the REAL committed bundles (durability replay of
 * genuine governed writer products). That path is recorded separately and is NOT the
 * initialization-seed contamination gate, which is evaluated at history formation. */
 ...
 *
 * `observableSituation` mirrors the production mechanism
 * (`ExplicitV4SessionAuthorityV0.admitFactualEvent`): when supplied, the SAME
 * committed `context` delta carries the observable scene/task, so the subject
 * actually RECEIVES the counterpart's message as its current observable scene.
 * Omitting it (the predecessor harness behavior) leaves `context.scene` at the
 * generic seeded task, and the model never sees the scenario at all. */
export async function admitObservation(
  ...
  const outcome = await runtime.ingressLedger.recordIngressEvent({
    ...
    subject_id: SUBJECT,
    conversation_id: "conv-familiarity-context-mediation",
    actor_ref: ALICE,
    text,
    logical_time: 0,
  ...
  const baseContextDelta = await buildContextDelta(observation as never, snapshot as never);
  const baseOp = baseContextDelta.operations[0] as unknown as { path: string; value: Record<string, unknown> };
  const contextDelta = observableSituation === undefined
    ? baseContextDelta
    : ({
        ...baseContextDelta,
        operations: [
  ...
  const commitOutcome = await runtime.assembly.facade.commitReserved({
    ...
    preparedBinding: {
      prepared_result_ref: `workflow:w-fam-ctxmed-obs-${sourceEventId}` as never,
