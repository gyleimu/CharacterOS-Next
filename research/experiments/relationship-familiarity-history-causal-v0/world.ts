...
/**
 * RELATIONSHIP_FAMILIARITY_HISTORY_CAUSAL_EXPERIMENT_V0 — world construction.
 *
 ...
 * mediator, one generic item, one distractor). Only the number of ADMITTED
 * familiarity receipts differs (plus the explicitly-named `E` working-ref
 * intervention) — familiarity is never assigned and never seeded; it accrues
 * through the real governed writer authority + Atomic Commit V2 path only.
 *
 * SEED TRUST BOUNDARY (required gate): every world records the exact
 * `observeSeededGovernedRelationshipStateV0` observation over the seeds actually
 * handed to the canonical store at INITIALIZATION. Familiarity must NOT exist
 * there:
 *
 ...
 *
 * If any of those fails, the experiment is INVALID (EXPERIMENT_INVALID_SEED_CONTAMINATION).
 */
...
export async function buildRepository(): Promise<{ repo: InMemoryMemoryRepository; binding: RepositoryRevisionBindingV1; records: readonly EpisodeRecordEntry[]; digest: string }> {
  ...
  const prepared = await repo.prepareRevisionForIntent({
    intent_id: "intent-familiarity-history-causal-corpus" as never,
...
export function workingRefsFor(condition: ConditionId): readonly string[] {
  // The availability control makes the mediator reachable WITHOUT familiarity, via an
  // explicit canonical working ref. The adversarial control makes the ENTIRE counterpart
  // corpus reachable WITHOUT familiarity. Every other cell keeps the identical baseline.
  if (condition === "F_LOW_FULL_CORPUS") return [GENERIC_REF, ...EPISODE_REFS];
  return condition === "E_LOW_WITH_MEDIATOR" ? [GENERIC_REF, MEDIATOR_REF] : [GENERIC_REF];
}
...
export async function buildHistoryWithCredits(
  ...
  const headProposal = {
    ...
    transition_id: `t-fam-history-head-${label}`,
    subject_id: SUBJECT,
  ...
  const committed = await assembly.facade.commitReserved({
    ...
    preparedBinding: {
      ...
      prepared_result_ref: `workflow:fam-history-head-${label}` as never
    },
...
export async function admitObservation(
  ...
  const outcome = await runtime.ingressLedger.recordIngressEvent({
    ...
    subject_id: SUBJECT,
    conversation_id: "conv-familiarity-history-causal",
    actor_ref: ALICE,
    text,
    logical_time: 0,
  ...
  const commitOutcome = await runtime.assembly.facade.commitReserved({
    ...
    preparedBinding: {
      prepared_result_ref: `workflow:w-fam-history-obs-${sourceEventId}` as never,
