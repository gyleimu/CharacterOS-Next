...
export interface ProviderPreflight {
  readonly schema_version: "canonical-affect-action-selection-provider-preflight-v0";
...
/** Read-only Ollama inventory, pinned to the one required model. */
export async function probeProviderEnvironment(): Promise<ProviderPreflight> {
  const frozen = await probeFrozenV1Provider();
  const primary = frozen.primary_model;
  return {
    schema_version: "canonical-affect-action-selection-provider-preflight-v0",
...
function firstPositionMetric(
  trials: readonly TrialRecord[],
  arm?: Arm
): { readonly selected: number; readonly eligible: number; readonly rate: number | null } {
  ...
  const selected = eligible.filter((trial) => trial.first_position_selected === true).length;
  return { selected, eligible: eligible.length, rate: rate(selected, eligible.length) };
}
...
function orderCellSummary(
  ...
  return {
    ...
    first_position_by_arm: Object.fromEntries(
      ARMS.map((arm) => [arm, firstPositionMetric(subset, arm)])
    ),
    first_position_aggregate: firstPositionMetric(subset)
  };
}
...
export function summarizeCollection(
  ...
  const scenarioSummary = SCENARIOS.map((scenario) => {
    const original = orderCellSummary(trials, scenario, "ORIGINAL");
    const reversed = orderCellSummary(trials, scenario, "REVERSED");
    const originalDirection = original.semantic_shift_direction;
    const reversedDirection = reversed.semantic_shift_direction;
    const orderInvariant =
      originalDirection !== "NONE" && originalDirection === reversedDirection;
    return {
      scenario_id: scenario.scenario_id,
      ...
      order_invariant_direction: orderInvariant ? originalDirection : "NONE"
    };
  });
  const orderInvariantScenarioCount = scenarioSummary.filter(
    (row) => row.order_invariant_semantic_effect === "YES"
  ).length;
  const firstPosition = firstPositionMetric(trials);
  const severePositionBias =
    firstPosition.rate !== null &&
    firstPosition.rate >= VERDICT_RULE.severe_first_position_rate;
  const scenarioBalanceFailure = phaseA.artifacts.phase_a["scenario_balance_documentation"] !== "PASS";
  const verdict = chooseVerdict({
    ...
    severe_position_bias: severePositionBias,
    scenario_balance_failure: scenarioBalanceFailure
  });
  ...
  const summary: Record<string, unknown> = {
    schema_version: "canonical-affect-action-selection-summary-v0",
    experiment_version: EXPERIMENT_VERSION,
    verdict,
    scientific_question:
    ...
    position_bias: {
      first_position_selection: firstPosition,
      severe_position_bias: severePositionBias,
      severe_threshold: VERDICT_RULE.severe_first_position_rate
    },
    ...
    claim_boundary: {
      ...
      canonical_affect_to_structured_action_selection: "THIS_EXPERIMENT_ONLY",
  ...
  const failureSummary: Record<string, unknown> = {
    schema_version: "canonical-affect-action-selection-failure-summary-v0",
    attempted_calls: trials.length,
