...
const snapshot = (await runtime.assembly.facade.readCurrentSnapshot("subject-familiarity-search-first" as never)) as unknown as {
  runtime_metadata: { state_revision: number };
};
const scene = await offlineScene(runtime, input.bundle.condition, 0, false);
