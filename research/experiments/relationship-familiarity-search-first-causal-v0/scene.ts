...
/**
 * RELATIONSHIP_FAMILIARITY_SEARCH_FIRST_CAUSAL_EXPERIMENT_V0 — one matched scene.
 *
 ...
 *
 * Conditions differ ONLY in: the canonical familiarity state (lawful history; LOW
 * 1/32 vs HIGH 16/32), therefore in whether the frozen influence triggers the
 * counterpart priority retrieval, and therefore in the retrieved context that
 * reaches Cognition. For `HIGH_SEARCH_ABLATED` the SAME HIGH state is used and the
 * priority retrieval CONTRIBUTION is suppressed (the production service still runs;
 * its result is replaced by an empty selection) — a RETRIEVAL_MEDIATOR_ABLATION.
 */
...
import { ALICE, MODEL, SUBJECT, type Scenario } from "./contract.ts";
...
export interface SceneObservation {
  readonly schema_version: "familiarity-search-first-scene-v0";
  readonly condition: string;
  ...
  readonly recognition: {
    ...
    readonly counterpart_evidence_in_prompt: boolean;
    readonly counterpart_handle: string | null;
  ...
  readonly retrieval: {
    ...
    readonly mediator_visible: boolean;
  };
  readonly cognition: {
    ...
    readonly retrieved_evidence_lines: readonly string[];
  };
  readonly language: {
    readonly calls: number;
...
export async function runScene(runtime: Runtime, input: SceneInput): Promise<SceneObservation> {
    ...
    retrieval: {
      retrieve: async (query: unknown) => {
        ...
        attempts.push({
          ...
          selected_refs: input.ablation ? [] : selected,
          selected_content_hashes: input.ablation ? [] : hashes,
          candidate_count: result.deterministic_metadata.candidate_count ?? null,
          replaced_with_empty: input.ablation
        });
        if (!input.ablation) return result;
        // RETRIEVAL_MEDIATOR_ABLATION: the production service still ran and produced a
        // validated result; only the familiarity-mediated CONTRIBUTION is suppressed.
  ...
  const requestId = `req-famsf-${input.condition}-${input.scenario.id}-r${input.replicate}`;
  const minter = createMiclStageMinter(runtime.assembly.facade as never, new InMemoryMiclWorkflowStore(), {
    micl_id: `micl-famsf-${requestId}` as never,
  ...
  const conventionHandle = /^-\s*(F\d+):\s*episode:alice-08$/m.exec(userContent)?.[1] ?? null;
  const counterpartEvidenceInPrompt = userContent.includes("Use concise wording, a factual tone, and no unnecessary apology");
  const retrievedEvidenceLines = userContent.split("\n").filter((line) => line.startsWith("- Past episode record (scene:"));
  ...
  const claims = (languagePayload?.["factual_assessment"] as { claims?: readonly { kind?: string; text?: string }[] } | undefined)?.claims ?? [];
  ...
  if (result.kind === "OUTPUT_READY") {
    ...
    else if (atomKind === "PRIMARY_FACT") {
      behaviorClass = designated?.kind === "SOURCE_QUOTE" && typeof designated.text === "string" && designated.text.length >= 20
  ...
  void MODEL;
  return {
    schema_version: "familiarity-search-first-scene-v0",
    ...
    recognition: {
      familiarity_lines: userContent.split("\n").filter((line) => /^- entity:alice: (presence|context_resolution_strategy)=/.test(line)),
      influence_lines: influenceEntryLine === null ? [] : [influenceEntryLine],
      counterpart_evidence_in_prompt: counterpartEvidenceInPrompt,
      counterpart_handle: conventionHandle,
      condition_label_leak: /\b(HIGH|LOW|ABLATED|condition=|experiment|replicate)\b/.test(userContent)
    },
    retrieval: {
      queries: queryCount,
      attempts,
      recent_retrieval_refs: recentRetrievalRefs,
      mediator_visible: counterpartEvidenceInPrompt && conventionHandle !== null
    },
    cognition: {
      ...
      retrieved_evidence_lines: retrievedEvidenceLines
    },
    language: {
