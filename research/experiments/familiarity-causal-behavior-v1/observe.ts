...
export interface Observation {
  ...
  // C2 added LanguageRealizationInputV4 (host-bound shape), C3 added V5 and C4 added V6; this
  // frozen experiment consumes the historical structured language input only.
  language: { input: Exclude<LanguageRealizationRequestV0["input"], { schema_version: "language-realization-input-v4" | "language-realization-input-v5" | "language-realization-input-v6" }>; input_hash: string; lawful_evidence_refs: string[] } | null;
...
export async function observeResponse(world: World, transports: { cognition: ModelTransportV0; language: ModelTransportV0 }, options: {
  ...
  function recordLanguage(provider: NonNullable<RuntimeDependencyContainer["languageRealizationProvider"]>) {
    ...
    provider.realize = async request => {
      o.language = { input: structuredClone(request.input) as Exclude<LanguageRealizationRequestV0["input"], { schema_version: "language-realization-input-v4" | "language-realization-input-v5" | "language-realization-input-v6" }>, input_hash: request.input_hash,
