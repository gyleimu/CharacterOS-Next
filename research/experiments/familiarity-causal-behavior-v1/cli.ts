...
function verifyFrozen(manifest: Manifest) {
...
async function run(manifestPath: string, outputPath: string, authorization: string | undefined) {
  ...
  const guard = async () => {
    verifyFrozen(manifest); check(git("status", "--porcelain") === "" && git("rev-parse", "HEAD") === head, "clean fixed source during primary");
...
check(first && second, "usage: cli.ts prepare <green-gates.json> <new-v1-readiness-dir> OR run <committed-manifest.json> <new-tmp-output> --authorize-formal-primary-v1");
if (operation === "prepare") await prepare(first, second);
else if (operation === "run") await run(first, second, authorization);
