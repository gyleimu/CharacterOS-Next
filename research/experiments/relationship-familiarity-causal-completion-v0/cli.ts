/**
 * RELATIONSHIP_FAMILIARITY_CAUSAL_COMPLETION_V0 — CLI.
 *
...
async function prepare(directory: string) {
  ...
  writeFileSync(join(directory, "SUMMARY.md"), [
    "# RELATIONSHIP_FAMILIARITY_CAUSAL_COMPLETION_V0 — readiness",
    "",
...
async function run(readinessDir: string, outputDir: string) {
  ...
  writeFileSync(join(outputDir, "SUMMARY.md"), [
    ...
    `Familiarity-material ablation directional (HIGH vs HIGH_ABLATED): ${verdict.ablation_directional}.`,
