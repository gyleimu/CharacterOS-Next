...
export function evaluatorMessages(reply: string, evidence: readonly EvidenceContent[]) {
  return [
    { role: "system" as const, content: EVALUATOR_SYSTEM },
    { role: "user" as const, content: JSON.stringify({ scenario: SCENARIO, reply, lawful_memory_evidence: evidence }) }
  ];
}
