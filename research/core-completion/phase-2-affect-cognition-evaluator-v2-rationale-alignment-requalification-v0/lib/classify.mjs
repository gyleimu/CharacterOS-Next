...
/**
 * rationale-classifier-v2 (upstream adjudication a636fb3): the value-framing family is
 * a lexical realization of PURE_PREFERENCE, not a new class. Bounded first-person
 * forms only; third-person / external-valuation frames ("Planning is objectively
 * valuable.", "This meeting has high value.") are deliberately NOT matched and stay
 * unlawful-as-unclassifiable. The forbidden side is byte-unchanged.
 */
export const RATIONALE_CLASSIFIER_VERSION = 'rationale-classifier-v2';
const PURE_PREFERENCE_VALUE_FRAMES = [
  /\bi (?:value|care about|favor|appreciate)\b/i,
  /\bi (?:find|consider)\b[^.]{0,30}\bworthwhile\b/i,
  /\bworthwhile to me\b|\bmatters to me\b|\bi would value\b/i
];
...
export function rationaleVerdict(rationale) {
  ...
  const allowed = [];
  const pushAllowed = (id) => { if (!allowed.includes(id)) allowed.push(id); };
  if (/\bi(?:'d| would)?\s+(?:rather|prefer)\b|\bi prefer\b|\bmy preference\b|\bprefer\b/i.test(text)) pushAllowed('PURE_PREFERENCE');
  if (PURE_PREFERENCE_VALUE_FRAMES.some((pattern) => pattern.test(text))) pushAllowed('PURE_PREFERENCE');
  if (/\bprioriti[sz]e\b|\bfirst\b|\bpriority\b|\brather (?:do|tackle) .* first\b/i.test(text)) pushAllowed('PRIORITY');
  if (/\brather not\b|\bprefer not\b|\bavoid\b|\bno (?:extra|more)\b/i.test(text)) pushAllowed('AVERSION');
  if (/\bwilling\b|\bglad to\b|\bhappy to\b|\bup for\b|\bdon't mind\b/i.test(text)) pushAllowed('WILLINGNESS');
  if (/\bapproach\b|\bstrategy\b|\bstyle\b|\bthoroughness\b|\bcooperative\b|\bproactive\b/i.test(text)) pushAllowed('SUBJECTIVE_STRATEGY');
