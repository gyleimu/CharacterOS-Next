/**
 * AFFECT_COGNITION_C4_4_SUBJECTIVE_BASIS_AND_STANCE_GROUNDING_V0 — frozen config.
 *
...
export const SUBJECT_ID = 'affect-cognition-contract-compaction-subject-v0';
...
export const PROTOCOL_STRINGS = Object.freeze({
  cognition: 'conversation-cognition-proposal-v6',
  language_input: 'language-realization-input-v7',
  ...
  applicability: 'SubjectiveSelectionV1 tagged applicability (NO_SUBJECTIVE_SELECTION | SUBJECTIVE_SELECTION), with the C4.4 latitude discriminator: a selection exists only when the facts and rules leave more than one behaviourally admissible, fact-compatible response',
  rationale: 'latent-subject-state policy: preference/priority/aversion/willingness/subjective strategy only',
  rationale_vocabulary: 'the rationale is a subjective frame (preference/priority/aversion/willingness/strategy) in which a supplied situational fact may be mentioned; it never states or restates a property of the subject',
  latitude: 'the latitude determination is a relation between the supplied facts/rules and the available responses, stated fact-relatively ("the supplied facts permit either volunteering or declining"), never as a subject property ("the subject has capacity for either response")',
  availability_not_capacity: 'a supplied fact that time or resources are available establishes only that fact; it does not establish capacity, capability, ability, energy, readiness, workload tolerance or any other subject-side condition, and no such mapping may be made',
  subject_property_terms: 'capacity, capability, capable, ability, able to, energy, stress, fatigue, bandwidth, readiness, ready to, workload tolerance, resilience — never a rationale, never the subject of a factual claim',
  citation_binding: 'every factual_assessment.claims[*].source_handles entry must be an advertised F handle and must also appear in cognition.considered_handles AND cognition.evidence_handles; the host binds the citation and refuses the turn otherwise',
  ref_handles: 'host-issued, turn-local, deterministic, wire-only handles: F* resolves in FACTUAL SOURCE REFS, C* in the remaining CITEABLE CONTEXT REFS (namespaces disjoint, one handle per ref); unknown, malformed, duplicate or namespace-escalating handles fail closed; canonical refs remain authoritative and are the only refs hashed or stored',
  version_policy: 'no schema or protocol version bump in this slice: the C4.4 protocol (conversation-cognition-proposal-v6 + language-realization-input-v7) is unchanged; the contract is compacted and the rationale examples neutralized, bound into this freeze by prompt digest'
});

/**
 * The frozen example set shipped in the compacted cognition contract (rules 5a/5b).
 * Positive examples are OUT-OF-DOMAIN: concrete enough to teach the lawful rationale
 * form, but containing no experimental stance-direction vocabulary, so a verbatim
 * copy cannot match any frozen stance class. Negative examples carry no decision
 * polarity either (subject-property statements). Frozen BEFORE any model call.
 */
export const POSITIVE_EXAMPLES = Object.freeze([
  'I prefer the window seat because I like the quieter side.',
  "I'd rather spend the available time reading.",
  "I'd rather use the blue notebook because it keeps my notes organized."
]);
export const NEGATIVE_EXAMPLES = Object.freeze([
  'I have enough capacity.',
  "I'm capable of doing it.",
  'My mind is fresh.'
]);
/** §21: directionally contaminated examples from earlier contracts that must be ABSENT. */
export const REMOVED_EXAMPLES = Object.freeze([
...
/** §20: frozen forbidden-experiment-vocabulary list audited against the positive set. */
export const FORBIDDEN_EXAMPLE_TOKENS = Object.freeze([
  'volunteer', 'decline', 'accept', 'refuse', 'try', 'keeping', 'keep the', 'stop', 'continue',
  'review', 'rehearse', 'inspect', 'first', 'second', 'attend', 'carry', 'polish', 'extra work',
  'package', 'meeting', 'presentation', 'backup', 'help', 'take it on', 'volunteering', 'declining'
]);

/** §28: preregistered transport-health endpoint. num_predict is frozen at 2048; a
 * completed cell may not exceed 90% of it, and no cell may end done_reason=length. */
export const HEADROOM = Object.freeze({ num_predict: 2048, threshold_90pct: 1843, threshold_80pct: 1638 });
...
export const CONTRACT_LEGIBILITY = Object.freeze({
  repaired_defect: 'factual claim source binding, now expressed in host-issued handles (claim source_handles ⊆ considered_handles ∩ evidence_handles)',
  prompt_rule: 'CITATION BINDING (C4.4) rule 7a, with a worked JSON example',
  host_validator_changed: false,
  additional_material_defects: 'none',
  known_minor_unstated_bounds: Object.freeze(['stance/rationale maximum 256 code points are implied by one-short-sentence wording but not stated numerically'])
});
