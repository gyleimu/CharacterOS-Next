/**
 * V7 factual-authority formal analysis (deterministic, zero model calls).
 *
...
const nullAnalysis = { schema_version: 'affect-cognition-c4-4-null-analysis-v0', per_scenario: {}, aggregate: { correct: 0, required: 0 }, pass: false };
...
const mixedAnalysis = { schema_version: 'affect-cognition-c4-4-mixed-analysis-v0', aggregate: { facts_correct: 0, selected: 0, preserved: 0, unsupported: 0, self_state: 0, total: 0 }, separation: {}, material_scenarios: [], pass: false };
...
const relevantAnalysis = { schema_version: 'affect-cognition-c4-4-relevant-analysis-v0', per_scenario: {}, material_scenarios: [], pass: false };
...
const historicalAnalysis = { schema_version: 'affect-cognition-c4-4-historical-analysis-v0', diagnostic_only: true, claim: 'no causal claim is made from S cells', per_scenario: {} };
...
const factualAudit = {
  schema_version: 'affect-cognition-factual-authority-formal-audit-v0',
  cells: rows.length,
  ...
  rejection_code_counts: Object.fromEntries([...new Set(rows.flatMap((row) => row.classification.factual_authorization_rejection_codes))].map((code) => [code, rows.filter((row) => row.classification.factual_authorization_rejection_codes.includes(code)).length])),
  pass: rows.length === 476 && rows.every((row) => row.classification.authoritative_runtime_factual_validity === true)
};
write('formal-factual-authority-audit.json', factualAudit);
const flags = {
  ...
  formal_factual_authority_pass: factualAudit.pass
};
let principal;
if (!flags.formal_factual_authority_pass) principal = 'AFFECT_COGNITION_FORMAL_FACTUAL_AUTHORITY_FAILED';
else if (flags.protocol_failures > rows.length * 0.1) principal = 'AFFECT_COGNITION_IMPLEMENTATION_FAILED';
else if (!flags.null_pass) principal = 'AFFECT_COGNITION_C4_4_SELECTION_APPLICABILITY_FAILED';
else if (flags.rationale_forbidden > 0) principal = 'AFFECT_COGNITION_C4_4_RATIONALE_BOUNDARY_FAILED';
else if (flags.off_question_stances > 0) principal = 'AFFECT_COGNITION_C4_4_STANCE_GROUNDING_FAILED';
else if (flags.self_state_facts > 0 || flags.unlawful_sources > 0) principal = 'AFFECT_COGNITION_C4_4_FACTUAL_ASSESSMENT_FAILED';
else if (flags.language_changed > 0 || flags.language_invented > 0 || flags.semantic_completions > 0) principal = 'AFFECT_COGNITION_C4_4_LANGUAGE_FIDELITY_FAILED';
else if (!flags.relevant_pass) principal = 'AFFECT_COGNITION_C4_4_SUBJECTIVE_DIFFERENTIATION_FAILED';
else if (flags.false_clarify > 0) principal = 'AFFECT_COGNITION_C4_4_CLARIFICATION_BOUNDARY_FAILED';
else if (flags.mixed_pass && flags.relevant_pass && flags.null_pass && flags.language_changed === 0) principal = 'AFFECT_COGNITION_C4_4_VALIDATED';
else principal = 'AFFECT_COGNITION_C4_4_REVALIDATION_INCONCLUSIVE';
write('verdict.json', { schema_version: 'affect-cognition-c4-4-formal-verdict-v0', rows: rows.length, flags, principal_verdict: principal });
