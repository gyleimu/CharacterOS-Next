 ...
 * production runtime. The cognition stub speaks the live V7 protocol (closed
 * factual authority, tagged selection, no model-emitted projection hash), and
 * the record keeps raw claims separate from the host-authorized V8 carrier.
 */
...
/** Valid V7 stub: empty factual set, tagged selection, no model-owned integrity. */
...
/** The V7 wire handles as the model saw them, plus the host's handle→ref map. */
...
function languageInputFromRequest(request) {
  ...
  const body = /LANGUAGE REALIZATION INPUT V8[\s\S]*?\n(\{[\s\S]*?\n\})\n/.exec(user)?.[1];
...
export async function runCondition({ snapshot, scenario, conditionId, cognitionTransport, languageTransport, sessionId }) {
  ...
  return {
    ...
    authoritative_subjective_selection: languageInput?.selected_subjective_selection ?? null,
    factual_assessment: proposal?.factual_assessment ?? null,
    raw_model_factual_assessment: proposal?.factual_assessment ?? null,
    authoritative_factual_assessment: languageInput?.factual_assessment ?? null,
    factual_authorization_trace: outcome.factual_authorization_trace ?? null,
    authoritative_proposal_minted: outcome.status === 'COMPLETE',
    authoritative_proposal_hash_minted: outcome.status === 'COMPLETE',
    ...
    stages: {
      ...
      SCHEMA_VALID: proposal?.schema_version === 'conversation-cognition-proposal-v7',
