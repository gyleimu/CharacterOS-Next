...
export function languageCompletionAudit(row, connectors) {
  const claims = (row.authoritative_factual_assessment?.claims ?? []).map((claim) => claim.text).join(' ');
  const stance = row.authoritative_subjective_selection?.kind === 'SUBJECTIVE_SELECTION' ? row.authoritative_subjective_selection.stance : '';
  const rationale = row.authoritative_subjective_selection?.kind === 'SUBJECTIVE_SELECTION' ? row.authoritative_subjective_selection.subjective_rationale ?? '' : '';
...
/** Runtime authority verdict comes only from the production policy trace. */
function factualAuthorityVerdict(record) {
  const rawClaims = Array.isArray(record.raw_model_factual_assessment?.claims)
    ? record.raw_model_factual_assessment.claims
    : [];
  const trace = Array.isArray(record.factual_authorization_trace)
    ? record.factual_authorization_trace
    : null;
  const rejected = trace?.filter((entry) => entry.status === 'REJECTED') ?? [];
  const traceComplete = trace !== null && trace.length === rawClaims.length;
  const rawCompliant = traceComplete && rejected.length === 0;
  const runtimeValid = rawCompliant;
  return {
    raw_compliant: rawCompliant,
    runtime_valid: runtimeValid,
    rejection_codes: [...new Set(rejected.map((entry) => entry.rejection_code ?? 'REJECTED_UNKNOWN'))],
    trace_complete: traceComplete
  };
}

export function classifyRecord(scenario, record, connectors) {
  const factualAuthority = factualAuthorityVerdict(record);
  const endpointSuppressed = !factualAuthority.runtime_valid || record.authoritative_proposal_minted !== true;
  const choiceRequired = scenario.choice !== undefined;
  const authoritativeSelection = record.authoritative_subjective_selection;
  const verdict = applicabilityVerdict(authoritativeSelection);
  const stance = verdict.tag === 'SUBJECTIVE_SELECTION' ? authoritativeSelection.stance : null;
  const rationale = verdict.tag === 'SUBJECTIVE_SELECTION' ? authoritativeSelection.subjective_rationale : null;
  const rationaleCheck = rationaleVerdict(rationale);
  const rawSelection = record.subjective_selection;
  const rawRationale = rawSelection?.kind === 'SUBJECTIVE_SELECTION' ? rawSelection.subjective_rationale : null;
  const rawRationaleCheck = rationaleVerdict(rawRationale);
  const finalText = record.final_behavior ?? '';
  const proposalText = record.raw_model_factual_assessment?.claims?.map((claim) => claim.text ?? '').join(' ') ?? '';
  const unsupported = unsupportedPremises(stance, rationale, record.current_intent, proposalText, finalText);
  const factualAudit = factualSelfStateAudit(record.raw_model_factual_assessment, record.advertised_handles);
  ...
  const base = {
    protocol: record.status === 'COMPLETE' && record.stages.SCHEMA_VALID && record.stages.LANGUAGE_ADMISSIBLE
      ? 'PASS'
      : factualAuthority.rejection_codes.length > 0 ? 'FACTUAL_AUTHORIZATION_REJECTED' : 'FAIL',
    raw_model_factual_compliance: factualAuthority.raw_compliant,
    authoritative_runtime_factual_validity: factualAuthority.runtime_valid,
    factual_authorization_rejection_codes: factualAuthority.rejection_codes,
    factual_authorization_trace_complete: factualAuthority.trace_complete,
    applicability: endpointSuppressed ? 'NOT_SCORED' : verdict.tag,
    applicability_reason: endpointSuppressed ? 'FACTUAL_AUTHORITY_INVALID' : verdict.reason,
    selection_applicability: endpointSuppressed ? 'NOT_SCORED' : (applicabilityCorrect ? 'SELECTION_APPLICABILITY_CORRECT' : 'SELECTION_APPLICABILITY_WRONG'),
    stance_selected: endpointSuppressed ? 'NOT_SCORED' : (stance === null ? 'NO_SUBJECTIVE_SELECTION' : stanceClass),
    off_question: endpointSuppressed ? 'NOT_SCORED' : (offQuestion ? 'OFF_QUESTION_STANCE' : (choiceRequired && verdict.tag === 'SUBJECTIVE_SELECTION' ? 'ON_QUESTION_STANCE' : 'NO_SUBJECTIVE_SELECTION')),
    ...
    runtime_authoritative_rationale_safe: endpointSuppressed ? null : rationaleCheck.lawful,
    raw_model_rationale_compliance: rawRationaleCheck.lawful,
    raw_model_rationale_category: rawRationaleCheck.category,
    raw_model_rationale_forbidden_classes: [...new Set(rawRationaleCheck.forbidden_classes.map((entry) => entry.kind))],
    ...
    proposal_fact: endpointSuppressed ? 'NOT_SCORED' : proposalFactClass(scenario, record.authoritative_factual_assessment),
  ...
  base.pass =
    base.protocol === 'PASS' &&
    base.authoritative_runtime_factual_validity === true &&
    base.raw_model_factual_compliance === true &&
