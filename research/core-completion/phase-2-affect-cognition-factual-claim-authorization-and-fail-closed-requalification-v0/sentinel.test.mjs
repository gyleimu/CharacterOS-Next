/**
 * V7 factual-authority deterministic transport sentinel (ZERO model calls).
 *
...
import { CONDITIONS, GROUNDING, MIXED_SCENARIOS, QUALIFICATION_SCENARIOS, RELEVANT_SCENARIOS } from './lib/config.mjs';
...
const R1 = RELEVANT_SCENARIOS.find((scenario) => scenario.id === 'R1');

function fakeCognition({ directive = 'REALIZE_CURRENT_INTENT', choice = { kind: 'SUBJECTIVE_SELECTION', stance: 'I would volunteer to own the review.', subjective_rationale: null }, clarifyChoice = { kind: 'NO_SUBJECTIVE_SELECTION' }, claimFactory = null } = {}) {
  const calls = [];
  return {
    calls,
    transport: {
      complete: async (request) => {
        ...
        const claims = claimFactory === null
          ? [{ kind: 'SOURCE_QUOTE', text: 'The code review deadline is Thursday.', source_handles: [observationHandle] }]
          : claimFactory(observationHandle);
        return {
          model: 'sentinel-fake',
  ...
  return {
    ...
    transport: {
      complete: async (request) => {
        calls.push(request);
        const user = request.messages.find((message) => message.role === 'user')?.content ?? '';
        const block = /LANGUAGE REALIZATION INPUT V8[\s\S]*?\n(\{[\s\S]*?\n\})\n/.exec(user)?.[1];
...
test('SELECTED sentinel: V7 facts, frozen selection policy, V8 handoff', async () => {
  ...
  const record = await runCondition({ snapshot, scenario: SCENARIO, conditionId: 'A', cognitionTransport: cognition.transport, languageTransport: language.transport, sessionId: 'sentinel-c44-selected' });
  ...
  assert.ok(system.includes('HOST_VERIFIABLE_DERIVATION'), 'the closed derived authority kind must be advertised');
  assert.ok(system.includes('INTEGER_ARITHMETIC'), 'the closed operation registry must be advertised');
  assert.ok(!system.includes('Use DERIVED_RESULT'), 'free-form derived authority must not be advertised');
  ...
  assert.equal(record.authoritative_proposal_minted, true);
  assert.equal(record.factual_authorization_trace[0].status, 'AUTHORIZED_SOURCE_QUOTE');
...
test('R1 free-form factual inference fails the whole proposal and suppresses Language/stance/hash', async () => {
  const snapshot = await growSnapshot(['Alice asks how the project documentation is currently organized.']);
  const cognition = fakeCognition({
    claimFactory: (handle) => [{
      kind: 'DERIVED_RESULT',
  ...
  const language = fakeLanguage();
  const record = await runCondition({ snapshot, scenario: R1, conditionId: 'A', cognitionTransport: cognition.transport, languageTransport: language.transport, sessionId: 'sentinel-fca-r1' });
  const classification = classifyRecord(R1, record, LANGUAGE_CONNECTORS);
  assert.equal(record.status, 'FAILED');
  assert.equal(record.authoritative_proposal_minted, false);
  assert.equal(record.authoritative_proposal_hash_minted, false);
  assert.equal(language.calls.length, 0);
  assert.equal(classification.authoritative_runtime_factual_validity, false);
  assert.equal(classification.stance_selected, 'NOT_SCORED');
  assert.deepEqual(classification.factual_authorization_rejection_codes, ['REJECTED_UNSUPPORTED_CLAIM_KIND']);
  assert.equal(record.factual_authorization_trace[0].raw_claim.kind, 'DERIVED_RESULT');
  assert.ok(Array.isArray(record.factual_authorization_trace[0].raw_claim.source_handles));
});
...
test('NOT_APPLICABLE sentinel: factual path completes, Language withholds preference', async () => {
  ...
  const language = fakeLanguage();
  const record = await runCondition({ snapshot, scenario: SCENARIO, conditionId: 'A', cognitionTransport: cognition.transport, languageTransport: language.transport, sessionId: 'sentinel-c44-not-applicable' });
...
test('default-off guard: an ungrounded stance is a RESEARCH verdict, never a host refusal', async () => {
  ...
  const language = fakeLanguage();
  const record = await runCondition({ snapshot, scenario: SCENARIO, conditionId: null, cognitionTransport: cognition.transport, languageTransport: language.transport, sessionId: 'sentinel-c44-guard-off' });
...
test('malformed tagged choices are still refused by the host before Language', async () => {
  ...
  for (const [label, choice] of [
    ...
    const language = fakeLanguage();
    const record = await runCondition({ snapshot, scenario: SCENARIO, conditionId: null, cognitionTransport: cognition.transport, languageTransport: language.transport, sessionId: `sentinel-c44-${label}` });
...
test('CLARIFY requires NOT_APPLICABLE and never calls Language', async () => {
  ...
  const language = fakeLanguage();
  const record = await runCondition({ snapshot, scenario: SCENARIO, conditionId: null, cognitionTransport: cognition.transport, languageTransport: language.transport, sessionId: 'sentinel-c44-clarify' });
  assert.equal(record.status, 'COMPLETE', `must complete: ${record.failure_detail}`);
  assert.equal(language.calls.length, 0);
  ...
  const failed = await runCondition({ snapshot, scenario: SCENARIO, conditionId: null, cognitionTransport: selected.transport, languageTransport: fakeLanguage().transport, sessionId: 'sentinel-c44-clarify-selected' });
