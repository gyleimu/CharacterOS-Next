...
import { authorizeSubjectiveRationaleV0, subjectiveFramePresentV0 } from '../../../../packages/runtime/dist/index.js';
...
/**
 * RATIONALE AUTHORIZATION — research adapter over the PRODUCTION policy
 * (subjective-rationale-authorization-policy-v0). The research audit consumes the
 * same deterministic implementation as the runtime; there is exactly one policy.
 * Field names are kept for the existing audits:
 *   category            -> AUTHORIZED | ABSENT | <rejection code>
 *   forbidden_classes   -> [{kind: <rejection code>}] for REJECTED (excluding NO_SUBJECTIVE_FRAME)
 *   allowed_classes     -> ['AUTHORIZED'] when authorized
 */
export const RATIONALE_VALIDATOR_VERSION = 'rationale-authorization-policy-v0 (research adapter)';

export function subjectiveFrame(rationale) {
  return rationale === null || rationale === undefined ? null : subjectiveFramePresentV0(rationale);
}

export function rationaleAuthorization(rationale) {
  return authorizeSubjectiveRationaleV0(rationale ?? null);
}

export function rationaleVerdict(rationale) {
  const authorization = authorizeSubjectiveRationaleV0(rationale ?? null);
  if (authorization.status === 'ABSENT') {
    return { category: 'ABSENT', lawful: true, allowed_classes: [], forbidden_classes: [], subjective_frame: null, authorization };
  }
  if (authorization.status === 'AUTHORIZED') {
    return { category: 'AUTHORIZED', lawful: true, allowed_classes: ['AUTHORIZED'], forbidden_classes: [], subjective_frame: true, authorization };
  }
  const reason = authorization.reason;
  const forbidden = reason === 'NO_SUBJECTIVE_FRAME' ? [] : [{ kind: reason }];
  return { category: reason, lawful: false, allowed_classes: [], forbidden_classes: forbidden, subjective_frame: reason !== 'NO_SUBJECTIVE_FRAME', authorization };
}
...
/**
 * n5q-result-evaluator-v2 (upstream adjudication a636fb3): a transformation answer is
 * contradictory only when a WRONG 4-character token is asserted in a RESULT ROLE.
 * Input mentions ("The input/token provided is R8K2") are lawful and never
 * contradictory. The bare copula "is" is deliberately NOT a result marker.
 */
export const N5Q_EVALUATOR_VERSION = 'n5q-result-evaluator-v2';
const N5Q_RESULT_ROLE_MARKERS = [
  /\b(?:result|output|answer)(?:\s+is)?\s+(?!2K8R\b)[A-Z0-9]{4}\b/i,
  /\breversed\s+(?:form\s+)?is\s+(?!2K8R\b)[A-Z0-9]{4}\b/i,
  /\breverses\s+to\s+(?!2K8R\b)[A-Z0-9]{4}\b/i,
  /\b(?:yields|becomes)\s+(?!2K8R\b)[A-Z0-9]{4}\b/i
];
...
export function factualClass(scenario, text) {
  ...
  if (scenario.id === 'N5Q' && N5Q_RESULT_ROLE_MARKERS.some((pattern) => pattern.test(stripped))) return 'FACTUAL_CONTRADICTION';
  const wrongPatterns = {
...
/**
 * choice-fidelity-evaluator-v2 (upstream adjudication 9eb0994): the delivered choice is
 * classified from the STANCE REALIZATION, not from all delivered prose. Deterministic
 * scope: the first sentence that carries a recognizable choice; hedging or conditionality
 * in later (rationale) sentences cannot reclassify the action. A true conditional choice
 * keeps its condition in the stance sentence and is still detected. If no sentence carries
 * a choice, fall back conservatively to the whole delivery so a choice expressed later is
 * never lost.
 */
export const CHOICE_EVALUATOR_VERSION = 'choice-fidelity-evaluator-v2';
export function stanceRealizationScope(delivered, kind) {
  if (kind === undefined) return { text: normalize(delivered), scope: 'NOT_CHOICE_BEARING' };
  const text = normalize(delivered);
  const sentences = text.split(/(?<=[.!?])\s+/).filter((sentence) => sentence.trim().length > 0);
  for (const sentence of sentences) {
    if (choiceClass(kind, sentence) !== 'NO_CHOICE') return { text: sentence, scope: 'FIRST_SENTENCE_WITH_CHOICE' };
  }
  return { text, scope: 'WHOLE_DELIVERY_FALLBACK' };
}
...
export function classifyRecord(scenario, record, connectors) {
  ...
  // DUAL ENDPOINTS: the runtime authority endpoint judges what the HOST authorized
  // and handed to Language; the raw-model endpoint reports what the model emitted
  // before authorization. They are never collapsed.
  const authoritativeSelection = record.language_selected_subjective_selection ?? null;
  const authoritativeRationale = authoritativeSelection?.kind === 'SUBJECTIVE_SELECTION' ? (authoritativeSelection.subjective_rationale ?? null) : null;
  const authoritativeRationaleCheck = rationaleVerdict(authoritativeRationale);
  const rawAuthorization = rationaleCheck.authorization;
  const rawRationaleForbidden = rationaleCheck.forbidden_classes.length > 0;
  const rationaleDropped = rawRationaleForbidden && authoritativeRationale === null;
  ...
  const choiceScope = choiceRequired ? stanceRealizationScope(finalText, scenario.choice) : { text: finalText, scope: 'NOT_CHOICE_BEARING' };
  const finalChoice = choiceRequired ? choiceClass(scenario.choice, choiceScope.text) : null;
  ...
  const base = {
    ...
    choice_evaluation_scope: choiceScope.scope,
    choice_scope_text: choiceScope.text,
    ...
    raw_rationale_status: rawAuthorization.status,
    raw_rationale_reason: rawAuthorization.reason ?? null,
    raw_rationale_forbidden: rawRationaleForbidden,
    authoritative_rationale: authoritativeRationale,
    authoritative_rationale_lawful: authoritativeRationaleCheck.lawful,
    rationale_dropped: rationaleDropped,
    runtime_authoritative_proposal_valid: record.status === 'COMPLETE' && record.stages?.SCHEMA_VALID === true,
  ...
  base.pass =
    ...
    (choiceRequired
      ? authoritativeRationaleCheck.lawful && !offQuestion && base.language_completion === 'PRESERVED' && base.language_choice === 'LANGUAGE_CHOICE_PRESERVED'
