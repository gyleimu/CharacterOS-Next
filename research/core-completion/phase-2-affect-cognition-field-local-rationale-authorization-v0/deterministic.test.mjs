/**
 * AFFECT_COGNITION_FIELD_LOCAL_RATIONALE_..._V0 — deterministic classifier tests. Zero model calls.
...
test('rationale authorization: lawful frames authorized, forbidden families rejected', () => {
  for (const text of [
    'I prefer the more expressive option.',
    'I value participating.',
    "I'd rather stop here.",
    'I consider participating worthwhile.'
  ]) {
    const verdict = rationaleVerdict(text);
    assert.equal(verdict.lawful, true, text);
    assert.deepEqual(verdict.allowed_classes, ['AUTHORIZED']);
  }
  const cases = [
    ...
    ['I only have five minutes.', 'UNSUPPORTED_EXTERNAL_FACT'],
    ['I handled this well before.', 'UNSUPPORTED_HISTORY_CLAIM'],
    ['This improves productivity.', 'NO_SUBJECTIVE_FRAME']
  ];
  for (const [text, expected] of cases) {
    const verdict = rationaleVerdict(text);
    assert.equal(verdict.lawful, false, text);
    assert.equal(verdict.category, expected, text);
  }
