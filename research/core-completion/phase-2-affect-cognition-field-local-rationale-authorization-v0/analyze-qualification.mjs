/**
 * AFFECT_COGNITION_FIELD_LOCAL_RATIONALE_..._V0 — qualification analysis (deterministic, zero model
...
const applicabilityAudit = {
  schema_version: 'affect-cognition-field-local-rationale-authorization-applicability-audit-v0', per_scenario: {},
...
const rationaleAudit = {
  schema_version: 'affect-cognition-field-local-rationale-authorization-rationale-audit-v0',
...
for (const row of rows) {
  ...
  if (forbidden.length > 0) {
    ...
    rationaleAudit.forbidden_cells.push({
      ...
      stance: row.subjective_selection?.kind === 'SUBJECTIVE_SELECTION' ? row.subjective_selection.stance : null,
      rationale: row.subjective_selection?.kind === 'SUBJECTIVE_SELECTION' ? row.subjective_selection.subjective_rationale : null,
...
const stanceAudit = {
  schema_version: 'affect-cognition-field-local-rationale-authorization-stance-grounding-audit-v0',
...
// ---- language fidelity ----------------------------------------------------------
const languageAudit = {
  schema_version: 'affect-cognition-field-local-rationale-authorization-language-audit-v0', counts: {}, completion_cells: [],
...
const factualAudit = {
  schema_version: 'affect-cognition-field-local-rationale-authorization-factual-authority-audit-v0', claims: 0, subject_state_fact_cells: 0,
...
const handleAudit = {
  schema_version: 'affect-cognition-field-local-rationale-authorization-handle-binding-audit-v0',
...
write('clarification-audit.json', { schema_version: 'affect-cognition-field-local-rationale-authorization-clarification-audit-v0', false_necessity: falseClarify, pass: falseClarify === 0 });
const leakageViolations = rows.filter((row) => row.language_leakage?.ok === false || row.request_attestation?.ok === false).length;
write('condition-leakage.json', { schema_version: 'affect-cognition-field-local-rationale-authorization-condition-leakage-v0', rows: rows.length, violations: leakageViolations, pass: leakageViolations === 0 });
...
const truncations = rows.filter((row) => (row.wrapper_error ?? '').includes('TRUNCATED') || (row.cognition_trace ?? []).some((event) => event?.ollama?.done_reason === 'length')).length;
const transportDegeneration = truncations > 0;
const implementationBroken = leakageViolations > 0
  || rows.some((row) => row.request_attestation === null || row.request_attestation === undefined);
const headroom = summary.headroom ?? null;
const flags = {
  implementation_intact: !implementationBroken,
  transport_degeneration: transportDegeneration,
  truncations,
  unreached_cells: unreached,
  headroom_pass: headroom === null ? false : headroom.pass === true,
  headroom,
  ...
  // The runtime authority endpoint: every authoritative rationale must be lawful, and
  // every raw-forbidden rationale must have been DROPPED by the host. A raw-forbidden
  // rationale reaching Language (authoritative rationale still non-null) is a host
  // authorization failure.
  rationale_authorization_pass: rows.every((row) => row.classification.authoritative_rationale_lawful === true
    && (!row.classification.raw_rationale_forbidden || row.classification.rationale_dropped === true)),
...
let principal;
if (transportDegeneration) principal = 'AFFECT_COGNITION_TRANSPORT_DEGENERATION_FAILED';
else if (!flags.rationale_authorization_pass) principal = 'AFFECT_COGNITION_RATIONALE_AUTHORIZATION_FAILED';
else if (!flags.implementation_intact) principal = 'AFFECT_COGNITION_IMPLEMENTATION_FAILED';
else if (!flags.handle_binding_pass) principal = 'AFFECT_COGNITION_REF_HANDLE_FAILED';
else if (!flags.rationale_pass) principal = 'AFFECT_COGNITION_RATIONALE_CONTRACT_FAILED';
else if (!flags.stance_grounding_pass) principal = 'AFFECT_COGNITION_APPLICABILITY_FAILED';
else if (!flags.language_pass) principal = 'AFFECT_COGNITION_LANGUAGE_FIDELITY_FAILED';
else if (!flags.factual_pass) principal = 'AFFECT_COGNITION_FACTUAL_ASSESSMENT_FAILED';
else if (falseClarify > 0) principal = 'AFFECT_COGNITION_CLARIFICATION_BOUNDARY_FAILED';
else if (flags.qualification_gate_passed) principal = 'AFFECT_COGNITION_VALIDATED';
else principal = 'AFFECT_COGNITION_REVALIDATION_INCONCLUSIVE';
const coPresent = [];
if (principal !== 'AFFECT_COGNITION_RATIONALE_CONTRACT_FAILED' && !flags.rationale_pass) coPresent.push('AFFECT_COGNITION_RATIONALE_CONTRACT_FAILED');
if (principal !== 'AFFECT_COGNITION_APPLICABILITY_FAILED' && !flags.stance_grounding_pass) coPresent.push('AFFECT_COGNITION_APPLICABILITY_FAILED');
if (principal !== 'AFFECT_COGNITION_LANGUAGE_FIDELITY_FAILED' && !flags.language_pass) coPresent.push('AFFECT_COGNITION_LANGUAGE_FIDELITY_FAILED');
const verdict = {
  schema_version: 'affect-cognition-field-local-rationale-authorization-qualification-verdict-v0',
