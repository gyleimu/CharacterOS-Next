/**
 * AFFECT_COGNITION_C4_3_..._V0 — deterministic transport sentinel (ZERO model calls).
 *
...
function fakeCognition({ directive = 'REALIZE_CURRENT_INTENT', choice = { kind: 'SELECTED', stance: 'I would volunteer to own the review.', subjective_rationale: null }, clarifyChoice = { kind: 'NOT_APPLICABLE' } } = {}) {
  const calls = [];
  return {
    calls,
    transport: {
      complete: async (request) => {
        ...
        assert.ok(refs.length > 0 && refs.includes(observationRef), 'sentinel: the current observation must be a factual source');
...
test('SELECTED sentinel: C4.2 policy prompts, tagged request, V6 handoff', async () => {
  ...
  const record = await runCondition({ snapshot, scenario: SCENARIO, conditionId: 'A', cognitionTransport: cognition.transport, languageTransport: language.transport, sessionId: 'sentinel-c43-selected' });
  ...
  assert.ok(system.includes('AND in cognition.evidence_refs'), 'the binding must name both cognition arrays');
  assert.ok(system.includes('must stay LATENT'), 'latent subject state must be stated');
  assert.ok(system.includes('The stance MUST answer the choice the user actually requested'), 'grounded-stance requirement must be present');
  ...
  assert.deepEqual(record.language_selected_subjective_choice, { kind: 'SELECTED', stance: 'I would volunteer to own the review.', subjective_rationale: null });
  const languageSystem = systemOf(language.calls[0]);
  assert.ok(languageSystem.includes('NO SEMANTIC COMPLETION'), 'the language no-completion rule must be present');
...
test('NOT_APPLICABLE sentinel: factual path completes, Language withholds preference', async () => {
  ...
  const language = fakeLanguage();
  const record = await runCondition({ snapshot, scenario: SCENARIO, conditionId: 'A', cognitionTransport: cognition.transport, languageTransport: language.transport, sessionId: 'sentinel-c43-not-applicable' });
...
test('default-off guard: an ungrounded stance is a RESEARCH verdict, never a host refusal', async () => {
  ...
  const cognition = fakeCognition({ choice: { kind: 'SELECTED', stance: 'I would instead consider something else entirely.', subjective_rationale: null } });
  const language = fakeLanguage();
  const record = await runCondition({ snapshot, scenario: SCENARIO, conditionId: null, cognitionTransport: cognition.transport, languageTransport: language.transport, sessionId: 'sentinel-c43-guard-off' });
...
test('malformed tagged choices are still refused by the host before Language', async () => {
  ...
  for (const [label, choice] of [
    ...
    const language = fakeLanguage();
    const record = await runCondition({ snapshot, scenario: SCENARIO, conditionId: null, cognitionTransport: cognition.transport, languageTransport: language.transport, sessionId: `sentinel-c43-${label}` });
...
test('CLARIFY requires NOT_APPLICABLE and never calls Language', async () => {
  ...
  const language = fakeLanguage();
  const record = await runCondition({ snapshot, scenario: SCENARIO, conditionId: null, cognitionTransport: cognition.transport, languageTransport: language.transport, sessionId: 'sentinel-c43-clarify' });
  assert.equal(record.status, 'COMPLETE', `must complete: ${record.failure_detail}`);
  ...
  const failed = await runCondition({ snapshot, scenario: SCENARIO, conditionId: null, cognitionTransport: selected.transport, languageTransport: fakeLanguage().transport, sessionId: 'sentinel-c43-clarify-selected' });
