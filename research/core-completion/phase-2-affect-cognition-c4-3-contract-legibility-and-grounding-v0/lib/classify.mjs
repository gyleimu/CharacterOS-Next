/**
 * AFFECT_COGNITION_C4_3_SUBJECTIVE_BASIS_AND_STANCE_GROUNDING_V0 — classifiers.
 *
...
/** C4.3 citation binding: every claim source must be in BOTH cognition arrays. */
export function citationBindingAudit(record) {
  const refs = record.proposal_cognition_refs;
  if (refs === null || refs === undefined) return { status: "NOT_APPLICABLE", unbound: [] };
  const considered = new Set(refs.considered_context_refs ?? []);
  const evidence = new Set(refs.evidence_refs ?? []);
  const unbound = [];
  for (const claim of record.factual_assessment?.claims ?? []) {
    for (const ref of claim.source_refs ?? []) {
      if (!considered.has(ref) || !evidence.has(ref)) {
        unbound.push({ ref, in_considered: considered.has(ref), in_evidence: evidence.has(ref) });
      }
    }
  }
  return { status: unbound.length === 0 ? "CITATION_BOUND" : "CITATION_UNBOUND", unbound };
}
...
export function languageCompletionAudit(row, connectors) {
  ...
  const stance = row.subjective_choice?.kind === 'SELECTED' ? row.subjective_choice.stance : '';
  const rationale = row.subjective_choice?.kind === 'SELECTED' ? row.subjective_choice.subjective_rationale ?? '' : '';
...
export function classifyRecord(scenario, record, connectors) {
  ...
  const applicabilityCorrect = choiceRequired ? verdict.tag === 'SELECTED' : verdict.tag === 'NOT_APPLICABLE';
  const stanceClass = choiceRequired && stance !== null ? choiceClass(scenario.choice, stance) : null;
  const offQuestion = choiceRequired && verdict.tag === 'SELECTED' && (stanceClass === 'NO_CHOICE');
  ...
  const citation = citationBindingAudit(record);
  ...
  const base = {
    ...
    stance_selected: stance === null ? 'NOT_APPLICABLE' : stanceClass,
    off_question: offQuestion ? 'OFF_QUESTION_STANCE' : (choiceRequired && verdict.tag === 'SELECTED' ? 'ON_QUESTION_STANCE' : 'NOT_APPLICABLE'),
    ...
    citation_binding: citation.status,
    citation_unbound_refs: citation.unbound,
    ...
    language_invented_preference: verdict.tag === 'SELECTED' ? false : preferencePresent(finalText)
  };
  ...
  base.pass =
    ...
    base.citation_binding !== 'CITATION_UNBOUND' &&
