...
/**
 * AFFECT_COGNITION_C4_CHOICE_APPLICABILITY_AND_SUBJECTIVE_BASIS_V0 — runtime driver.
 *
 ...
 * (`conversation-cognition-proposal-v5`: tagged choice, no model-emitted
 * projection_hash), and the record captures the tagged choice plus the V6
 * language handoff (`selected_subjective_choice`).
 */
...
export function stubCognitionTransport() {
  return { complete: async () => ({ model: 'stub', content: JSON.stringify({
    ...
    subjective_choice: { kind: 'SELECTED', stance: 'I would respond directly to the current request.', subjective_rationale: null },
...
/** The V6 language input as the model saw it. */
...
export async function runCondition({ snapshot, scenario, conditionId, cognitionTransport, languageTransport, sessionId }) {
  ...
  const proposalCognitionRefs = proposal?.cognition === undefined ? null : {
    considered_context_refs: Array.isArray(proposal.cognition.considered_context_refs) ? proposal.cognition.considered_context_refs : [],
    evidence_refs: Array.isArray(proposal.cognition.evidence_refs) ? proposal.cognition.evidence_refs : []
  };
  ...
  return {
    ...
    proposal_cognition_refs: proposalCognitionRefs,
    ...
    stages: {
      ...
      SCHEMA_VALID: proposal?.schema_version === 'conversation-cognition-proposal-v5',
