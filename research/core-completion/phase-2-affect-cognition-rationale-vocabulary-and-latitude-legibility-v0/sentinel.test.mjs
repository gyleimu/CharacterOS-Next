/**
 * AFFECT_COGNITION_RATIONALE_VOCABULARY_..._V0 — deterministic transport sentinel (ZERO model calls).
 *
...
test('SELECTED sentinel: C4.2 policy prompts, tagged request, V6 handoff', async () => {
  ...
  assert.ok(system.includes('must stay LATENT'), 'latent subject state must be stated');
  ...
  // Vocabulary-legibility repair: negative AND positive examples, the
  // availability != capacity rule, and the fact-relative latitude form.
  for (const lawful of ["I'd rather help.", 'I prefer to use the free time to help.', "I'd be willing to spend the available time on the review."]) {
    assert.ok(system.includes(lawful), `lawful worked example must be present: ${lawful}`);
  }
  for (const unlawful of ['I have available capacity.', 'My energy is high enough.', 'My mind is fresh.', 'I can manage the workload.', 'The subject has sufficient workload tolerance.']) {
    assert.ok(system.includes(unlawful), `unlawful worked example must be present: ${unlawful}`);
  }
  assert.ok(system.includes('AVAILABILITY IS NOT CAPACITY'), 'the availability != capacity rule must be stated');
  assert.ok(system.includes('it does not establish that the subject has capacity'), 'the no-mapping rule must be stated');
  assert.ok(system.includes('the supplied facts permit either volunteering or declining'), 'the fact-relative latitude example must be present');
  ...
  assert.ok(system.includes('The subject has capacity.'), 'the forbidden subject-property claim must be named');
  assert.ok(system.includes('The supplied facts leave both volunteering and declining feasible.'), 'the lawful fact-relative claim must be named');
...
test('frozen mechanics are unchanged by the vocabulary repair', async () => {
  ...
  const cognition = fakeCognition();
  const language = fakeLanguage();
  const record = await runCondition({ snapshot, scenario: SCENARIO, conditionId: 'A', cognitionTransport: cognition.transport, languageTransport: language.transport, sessionId: 'sentinel-rv-frozen' });
  const system = systemOf(record.raw_cognition_request);
