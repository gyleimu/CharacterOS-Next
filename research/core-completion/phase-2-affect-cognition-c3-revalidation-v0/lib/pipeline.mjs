...
/**
 * AFFECT_COGNITION_C3_REVALIDATION_V0 — production runtime driver.
 *
 * The condition is applied at the transport boundary against the UNMODIFIED
 * production runtime. The cognition stub now speaks the C3 protocol
 * (`conversation-cognition-proposal-v4`: explicit `subjective_choice`, no
 * model-emitted `projection_hash`), and the record captures both the explicit
 * choice and the V5 language handoff (`selected_subjective_choice`).
 */
...
/** Valid C3 (V4) stub proposal: semantics only, explicit choice, host-owned hash. */
...
export async function growSnapshot(events) {
  const runtime = await InteractiveSubjectRuntimeV0.create(runtimeOptions(stubCognitionTransport(), `c3-${randomUUID()}`, stubLanguageTransport()));
...
/** The V5 language input as the model saw it (data block after the header line). */
function languageInputFromRequest(request) {
  const user = request?.messages?.find((message) => message.role === 'user')?.content ?? '';
  const body = /LANGUAGE REALIZATION INPUT V5[\s\S]*?\n(\{[\s\S]*?\n\})\n/.exec(user)?.[1];
  return parseJson(body);
}

/**
 * Language-stage isolation audit for the V5 handoff: no scenario/condition
 * identity, no model-visible integrity hash, no raw subject state, and no
 * descriptive `current_intent` (V5 replaced it with the explicit choice).
 */
function languageLeakage(request, scenario) {
  ...
  if (parsed !== null) {
    ...
    if (!Object.hasOwn(parsed, 'selected_subjective_choice')) found.push('explicit_choice_missing_from_language_input');
  }
...
export async function runCondition({ snapshot, scenario, conditionId, cognitionTransport, languageTransport, sessionId }) {
  ...
  return {
    ...
    subjective_choice: proposal?.subjective_choice ?? null,
    language_selected_subjective_choice: languageInput?.selected_subjective_choice ?? null,
    ...
    stages: {
      ...
      SCHEMA_VALID: proposal?.schema_version === 'conversation-cognition-proposal-v4',
