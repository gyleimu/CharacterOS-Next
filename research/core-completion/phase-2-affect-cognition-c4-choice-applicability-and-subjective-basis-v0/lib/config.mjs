/**
 * AFFECT_COGNITION_C4_CHOICE_APPLICABILITY_AND_SUBJECTIVE_BASIS_V0 — frozen config.
 *
 * Same scenario set, provider baseline, conditions, materiality floors and
 * condition-attestation machinery as the C3 clean revalidation, so C4 is
 * comparable cell-for-cell. The protocol surfaces advance to the C4 versions:
 * conversation-cognition-proposal-v5 (tagged choice applicability + bounded
 * subjective basis, host-bound projection hash) and language-realization-input-v6.
 */
...
export const SUBJECT_ID = 'affect-cognition-c4-subject-v0';
...
export const PROTOCOL_STRINGS = Object.freeze({
  ...
  rationale: 'bounded non-factual subjective_rationale (preference/priority/aversion/willingness/subjective strategy)'
});

/** The C4 choice-applicability endpoint, per the frozen architecture decision. */
export const APPLICABILITY_RULES = Object.freeze({
  null_scenarios: 'NOT_APPLICABLE',
  choice_scenarios: 'SELECTED',
  host_validates: 'shape only (tag/branch discipline); correctness is enforced by this qualification, never by a production task taxonomy',
  clarify: 'CLARIFY_MISSING_CONTEXT requires NOT_APPLICABLE'
});

/**
 * Family-D falsification thresholds, both mandatory:
 *   failure-to-select   — a choice-bearing scenario that stops selecting;
 *   failure-to-withhold — a null scenario that keeps declaring a selection.
 */
...
/** Subjective-basis routing failure: subject-side reasoning still entering facts. */
export const SUBJECTIVE_BASIS_RULE = Object.freeze({
  min_scenarios: 2,
  vocabulary: Object.freeze(['capacity', 'energy', 'stress', 'fatigue', 'state_revision', 'regulation', 'exhaust', 'tired'])
});
