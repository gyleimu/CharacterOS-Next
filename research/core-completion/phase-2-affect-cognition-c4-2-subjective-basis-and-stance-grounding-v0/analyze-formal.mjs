/**
 * AFFECT_COGNITION_C4_2_CHOICE_APPLICABILITY_AND_SUBJECTIVE_BASIS_V0 —
...
const nullAnalysis = { schema_version: 'affect-cognition-c4-2-null-analysis-v0', per_scenario: {}, aggregate: { correct: 0, required: 0 }, pass: false };
for (const scenario of NULL_SCENARIOS) {
  ...
  for (const condition of conditions) {
    ...
    const correct = cells.filter((row) => row.classification.protocol === 'PASS' && row.classification.final_fact === 'FACT_CORRECT'
      && row.classification.applicability === 'NOT_APPLICABLE' && row.classification.language_invented_preference === false
...
const mixedAnalysis = { schema_version: 'affect-cognition-c4-2-mixed-analysis-v0', aggregate: { facts_correct: 0, selected: 0, preserved: 0, unsupported: 0, self_state: 0, total: 0 }, separation: {}, material_scenarios: [], pass: false };
for (const scenario of MIXED_SCENARIOS) {
  for (const condition of conditions) {
    ...
    mixedAnalysis.aggregate.selected += cells.filter((row) => row.classification.applicability === 'SELECTED').length;
...
const relevantAnalysis = { schema_version: 'affect-cognition-c4-2-relevant-analysis-v0', per_scenario: {}, material_scenarios: [], pass: false };
...
const historicalAnalysis = { schema_version: 'affect-cognition-c4-2-historical-analysis-v0', diagnostic_only: true, claim: 'no causal claim is made from S cells', per_scenario: {} };
for (const scenario of HISTORICAL_SCENARIOS) {
  for (const condition of conditions) {
    const cells = byScenario(scenario.id).filter((row) => row.condition === condition);
    historicalAnalysis.per_scenario[`${scenario.id}/${condition}`] = { cells: cells.length, tags: distribution(cells.map((row) => row.classification.applicability), ['NOT_APPLICABLE', 'SELECTED', 'UNLAWFUL']), unsupported: cells.reduce((sum, row) => sum + row.classification.unsupported_premises.length, 0) };
  }
}
...
let principal;
if (flags.protocol_failures > rows.length * 0.1) principal = 'AFFECT_COGNITION_C4_2_IMPLEMENTATION_FAILED';
else if (!flags.null_pass) principal = 'AFFECT_COGNITION_C4_2_CHOICE_APPLICABILITY_FAILED';
else if (flags.rationale_forbidden > 0) principal = 'AFFECT_COGNITION_C4_2_RATIONALE_BOUNDARY_FAILED';
else if (flags.off_question_stances > 0) principal = 'AFFECT_COGNITION_C4_2_STANCE_GROUNDING_FAILED';
else if (flags.self_state_facts > 0 || flags.unlawful_sources > 0) principal = 'AFFECT_COGNITION_C4_2_FACTUAL_ASSESSMENT_FAILED';
else if (flags.language_changed > 0 || flags.language_invented > 0 || flags.semantic_completions > 0) principal = 'AFFECT_COGNITION_C4_2_LANGUAGE_FIDELITY_FAILED';
else if (!flags.relevant_pass) principal = 'AFFECT_COGNITION_C4_2_SUBJECTIVE_DIFFERENTIATION_FAILED';
else if (flags.false_clarify > 0) principal = 'AFFECT_COGNITION_C4_2_CLARIFICATION_BOUNDARY_FAILED';
else if (flags.mixed_pass && flags.relevant_pass && flags.null_pass && flags.language_changed === 0) principal = 'AFFECT_COGNITION_C4_2_VALIDATED';
else principal = 'AFFECT_COGNITION_C4_2_REVALIDATION_INCONCLUSIVE';
write('verdict.json', { schema_version: 'affect-cognition-c4-2-formal-verdict-v0', rows: rows.length, flags, principal_verdict: principal });
