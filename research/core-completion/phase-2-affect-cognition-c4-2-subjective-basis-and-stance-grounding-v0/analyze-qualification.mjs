/**
 * AFFECT_COGNITION_C4_2_..._V0 — qualification analysis (deterministic, zero model
...
const applicabilityAudit = {
  schema_version: 'affect-cognition-c4-2-applicability-audit-v0', per_scenario: {},
...
const rationaleAudit = {
  schema_version: 'affect-cognition-c4-2-rationale-audit-v0',
...
const stanceAudit = {
  schema_version: 'affect-cognition-c4-2-stance-grounding-audit-v0',
...
// ---- language fidelity ----------------------------------------------------------
const languageAudit = {
  schema_version: 'affect-cognition-c4-2-language-audit-v0', counts: {}, completion_cells: [],
...
languageAudit.pass = languageAudit.completion_cells.length === 0 && languageAudit.invented_preference_cells === 0
  ...
  && (languageAudit.counts.LANGUAGE_CHOICE_CHANGED ?? 0) === 0 && (languageAudit.counts.LANGUAGE_CHOICE_DROPPED ?? 0) === 0;
write('language-audit.json', languageAudit);

// ---- factual authority ----------------------------------------------------------
const baseRequests = read('base-requests.json');
const factualAudit = {
  schema_version: 'affect-cognition-c4-2-factual-authority-audit-v0', claims: 0, subject_state_fact_cells: 0,
...
write('clarification-audit.json', { schema_version: 'affect-cognition-c4-2-clarification-audit-v0', false_necessity: falseClarify, pass: falseClarify === 0 });
const leakageViolations = rows.filter((row) => row.language_leakage?.ok === false || row.request_attestation?.ok === false).length;
write('condition-leakage.json', { schema_version: 'affect-cognition-c4-2-condition-leakage-v0', rows: rows.length, violations: leakageViolations, pass: leakageViolations === 0 });
...
const falsification = {
  schema_version: 'affect-cognition-c4-2-family-d-falsification-v0', threshold: FALSIFICATION,
...
const implementationBroken = leakageViolations > 0 || unreached > 0 || !applicabilityAudit.pass
...
let principal;
if (!flags.implementation_intact) principal = 'AFFECT_COGNITION_C4_2_IMPLEMENTATION_FAILED';
else if (!flags.rationale_pass) principal = 'AFFECT_COGNITION_C4_2_RATIONALE_BOUNDARY_FAILED';
else if (!flags.stance_grounding_pass) principal = 'AFFECT_COGNITION_C4_2_STANCE_GROUNDING_FAILED';
else if (!flags.language_pass) principal = 'AFFECT_COGNITION_C4_2_LANGUAGE_FIDELITY_FAILED';
else if (!flags.factual_pass) principal = 'AFFECT_COGNITION_C4_2_FACTUAL_ASSESSMENT_FAILED';
else if (falseClarify > 0) principal = 'AFFECT_COGNITION_C4_2_CLARIFICATION_BOUNDARY_FAILED';
else if (flags.qualification_gate_passed) principal = 'AFFECT_COGNITION_C4_2_VALIDATED';
else principal = 'AFFECT_COGNITION_C4_2_REVALIDATION_INCONCLUSIVE';
const coPresent = [];
if (principal !== 'AFFECT_COGNITION_C4_2_RATIONALE_BOUNDARY_FAILED' && !flags.rationale_pass) coPresent.push('AFFECT_COGNITION_C4_2_RATIONALE_BOUNDARY_FAILED');
if (principal !== 'AFFECT_COGNITION_C4_2_STANCE_GROUNDING_FAILED' && !flags.stance_grounding_pass) coPresent.push('AFFECT_COGNITION_C4_2_STANCE_GROUNDING_FAILED');
if (principal !== 'AFFECT_COGNITION_C4_2_LANGUAGE_FIDELITY_FAILED' && !flags.language_pass) coPresent.push('AFFECT_COGNITION_C4_2_LANGUAGE_FIDELITY_FAILED');
const verdict = {
  schema_version: 'affect-cognition-c4-2-qualification-verdict-v0',
