/**
 * AFFECT_COGNITION_INSTRUMENT_ALIGNMENT_..._V0 — deterministic transport sentinel (ZERO model calls).
 *
...
test('SELECTED sentinel: C4.2 policy prompts, tagged request, V6 handoff', async () => {
  ...
  const record = await runCondition({ snapshot, scenario: SCENARIO, conditionId: 'A', cognitionTransport: cognition.transport, languageTransport: language.transport, sessionId: 'sentinel-ia-selected' });
...
test('NOT_APPLICABLE sentinel: factual path completes, Language withholds preference', async () => {
  ...
  const language = fakeLanguage();
  const record = await runCondition({ snapshot, scenario: SCENARIO, conditionId: 'A', cognitionTransport: cognition.transport, languageTransport: language.transport, sessionId: 'sentinel-ia-not-applicable' });
...
test('default-off guard: an ungrounded stance is a RESEARCH verdict, never a host refusal', async () => {
  ...
  const language = fakeLanguage();
  const record = await runCondition({ snapshot, scenario: SCENARIO, conditionId: null, cognitionTransport: cognition.transport, languageTransport: language.transport, sessionId: 'sentinel-ia-guard-off' });
...
test('malformed tagged choices are still refused by the host before Language', async () => {
  ...
  for (const [label, choice] of [
    ...
    const language = fakeLanguage();
    const record = await runCondition({ snapshot, scenario: SCENARIO, conditionId: null, cognitionTransport: cognition.transport, languageTransport: language.transport, sessionId: `sentinel-ia-${label}` });
...
test('CLARIFY requires NOT_APPLICABLE and never calls Language', async () => {
  ...
  const language = fakeLanguage();
  const record = await runCondition({ snapshot, scenario: SCENARIO, conditionId: null, cognitionTransport: cognition.transport, languageTransport: language.transport, sessionId: 'sentinel-ia-clarify' });
  assert.equal(record.status, 'COMPLETE', `must complete: ${record.failure_detail}`);
  ...
  const failed = await runCondition({ snapshot, scenario: SCENARIO, conditionId: null, cognitionTransport: selected.transport, languageTransport: fakeLanguage().transport, sessionId: 'sentinel-ia-clarify-selected' });
...
test('frozen mechanics are unchanged by the vocabulary repair', async () => {
  ...
  const cognition = fakeCognition();
  const language = fakeLanguage();
  const record = await runCondition({ snapshot, scenario: SCENARIO, conditionId: 'A', cognitionTransport: cognition.transport, languageTransport: language.transport, sessionId: 'sentinel-ia-frozen' });
  const system = systemOf(record.raw_cognition_request);
