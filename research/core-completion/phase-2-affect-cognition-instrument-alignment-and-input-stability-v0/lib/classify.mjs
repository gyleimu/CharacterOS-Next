...
/**
 * rationale-validator-v3 (upstream adjudication 9eb0994): MINIMAL SUBJECTIVE FRAME +
 * FORBIDDEN BOUNDARY. The positive side no longer enumerates a rationale vocabulary; it
 * asks only whether the text presents itself as THIS subject's evaluation, attitude,
 * valuation, willingness, priority, aversion or strategy. The forbidden families are
 * byte-unchanged and always win. Arbitrary first-person text without an evaluative frame
 * does not pass; NO_SUBJECTIVE_FRAME replaces the old UNCLASSIFIED meaning.
 */
export const RATIONALE_VALIDATOR_VERSION = 'rationale-validator-v3';

/** Descriptive named-family detectors (labels only; they do not gate lawfulness). */
const NAMED_FAMILIES = [
  ['PURE_PREFERENCE', /\bi(?:'d| would)?\s+(?:rather|prefer)\b|\bi prefer\b|\bmy preference\b|\bprefer\b/i],
  ['PRIORITY', /\bprioriti[sz]e\b|\bpriority\b/i],
  ['AVERSION', /\brather not\b|\bprefer not\b|\bavoid\b|\bno (?:extra|more)\b/i],
  ['WILLINGNESS', /\bwilling\b|\bglad to\b|\bhappy to\b|\bup for\b|\bdon't mind\b/i],
  ['SUBJECTIVE_STRATEGY', /\bapproach\b|\bstrategy\b|\bstyle\b|\bthoroughness\b|\bcooperative\b|\bproactive\b/i]
];

/** The minimal subjective frame: a deterministically checkable subject-relative
 * evaluative/attitude construction. Structural, not an ontology of reasons. */
const SUBJECTIVE_FRAME_PATTERNS = [
  /\bi(?:'d| would)?\s+(?:rather|prefer)\b/i,
  /\bi (?:value|care about|favor|favour|appreciate|enjoy|like|love)\b/i,
  /\bi (?:find|consider)\b[^.]{0,40}\b(?:useful|worthwhile|valuable|important|beneficial|helpful|meaningful|rewarding|enjoyable|pleasing|wise|sensible)\b/i,
  /\bi(?:'d|'m| am| would| will)?\s*(?:be\s+)?(?:willing|glad|happy|eager)\s+to\b/i,
  /\bi (?:believe|think|regard|see)\b[^.]{0,40}\b(?:better|best|right|worthwhile|valuable|important|useful|preferable|sensible|wise)\b/i,
  /\bi (?:prioriti[sz]e|opt for|avoid|favour|favor)\b/i,
  /\b(?:matters|means something|counts|is important|is valuable|is worthwhile)\s+to me\b/i
];
...
/**
 * Classifies one rationale. LAWFUL iff (1) no forbidden semantic class fires and
 * (2) a minimal subjective frame is present. NO_SUBJECTIVE_FRAME replaces the old
 * UNCLASSIFIED meaning; it remains a failure (fail-safe preserved).
 */
export function rationaleVerdict(rationale) {
  if (rationale === null || rationale === undefined) {
    return { category: 'ABSENT', lawful: true, allowed_classes: [], forbidden_classes: [], subjective_frame: null };
  }
  const text = normalize(rationale);
  const forbidden = [
  ...
  const frame = subjectiveFrame(text);
  const named = NAMED_FAMILIES.filter(([, pattern]) => pattern.test(text)).map(([id]) => id);
  const forbiddenKinds = [...new Set(forbidden.map((entry) => entry.kind))];
  const lawful = forbiddenKinds.length === 0 && frame;
  const category = forbiddenKinds.length > 0 ? forbiddenKinds[0]
    : frame ? (named[0] ?? 'SUBJECTIVE_FRAME') : 'NO_SUBJECTIVE_FRAME';
  return {
    category, lawful,
    allowed_classes: frame ? (named.length > 0 ? named : ['SUBJECTIVE_FRAME']) : [],
    forbidden_classes: forbidden, subjective_frame: frame
  };
}
